...
function gg(gtgdeagle, gtgshotgun, gtgsmg, gtgak47, gtgm4a1, gtgrifle)
	...
	if countgg > 0 then
		...
		if string.find(bikernick, "_") then
			...
			sampSendChat("/f [Сторож]: "..bikername.." "..bikersurname.." взял со склада оружия на "..countgg.." материалов!")
		else
			sampSendChat("/f [Сторож]: "..bikernick.." взял со склада оружия на "..countgg.." материалов!")
		end
	end
	countgg = 0
end
...
function ggid(getgunider, gtgdeagle, gtgshotgun, gtgsmg, gtgak47, gtgm4a1, gtgrifle)
	...
	if countgg > 0 then
		countgg = countgg
		if string.find(ggidnick, "_") then
			sampSendChat("/f [Сторож]: "..ggidname.."'у "..ggidsurname.."'у было выдано "..countgg.." материалов со склада!")
		else
			sampAddChatMessage("/f [Сторож]: "..ggidnick.."'у было выдано "..countgg.." материалов со склада!")
		end
	end
	countgg = 0
end
